...
Y si los archivos ya existían, por la persistencia de datos, pues el constructor los lee y almacena datos importantes como la cantidad de buckets sin overlflow y la cantidad de buckets con overflow.  
   ```cpp
   template<typename T>
   ...
   ```
#### Insert
La función `insert` está diseñada para seguir pasos para caso posible: sin overflow, con overflow o con encadenamiento. 

<p align="center">
  <img src="imagenes/eh.insertar.png" alt="Insert - EH">
</p> 

1. Primero obtenemos el *hash_index* de la key del Record, para así poder obtener el *bucked_id* según ese hash, teniendo en cuenta la profundidad del hash (que sea <= D (profundidad global)).
   ```cpp
   ...
   ```
2.  Con la función **get_bucket_id** buscamos el id correspondiente al bucket asignado al hash que coincide con el hash del key. Ésta búsqueda se realiza en el archivo ***address_table.dat***,el cual tiene la estructura:   
...
3. Dentro de esta función hacemos un llamado a la función auxiliar **same_index** la cual nos regresa el booleano de la relación entre el hash_index ingresado y el hash_index relacionado a cada bucket_id en **address_table.dat**.  
   ...
   ```
4. Una vez obtenido el bucket_id, podemos entrar al archivo "hashfile.dat" para conocer en qué situación de inserción nos encontramos, teniendo en cuenta que éste tiene la siguiente estructura:
<p align="center">
...
- Aquí hicimos una llamada a la función .split() de la struct del ExtendibleHashing, la cual retorna el bucket_id del nuevo bucket generado al splitear, ya que por dentro al hash_index que se mantiene le desbloquea un bit por delante el cual es '0', y el nuevo AdressRecord tiene el mismo hash_index, con la diferencia de en lugar de desbloquear el '0', desbloquea el '1', ya que así lo requiere el split.
   ...
   ```
**Caso 3:** ***con  overflow de encademaiento***  
